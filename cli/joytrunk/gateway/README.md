...
- 默认监听 `127.0.0.1:32891`（由配置 `gateway.a2a_port` 决定）。
- 默认配置已包含 `gateway.a2a_port: 32891`，无需改配置即可使用。
- 启动后日志中会看到类似：`A2A Gateway listening on 127.0.0.1:32891`。
...
2. Server：根据配置中的 `gateway.a2a_port` 或 `gateway.a2a_backend_url` 请求 `http://127.0.0.1:32891/a2a/v1/tenants/.../message:send`
